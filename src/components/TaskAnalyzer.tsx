    ...
    <div className="max-h-[70vh] overflow-y-auto py-4">
      ...
      {showMergeConfirmationView && analysisResults && (
        <div className="space-y-6">
          {/* Fixed header with merge action controls */}
          <div className="sticky top-0 z-10 bg-white dark:bg-gray-900 pb-4 border-b border-gray-200 dark:border-gray-700 shadow-sm">
            <div className="mb-4">
            ...
            <div className="flex justify-between items-center py-2">
              <div className="text-sm text-gray-600 dark:text-gray-300">
                <span className="font-medium">{selectedMergeCount()}</span> of <span className="font-medium">{countMergeEligibleDuplicates() + countMergeEligibleSimilar()}</span> suggestions selected
              </div>
              <div className="flex space-x-4">
                <button
                  onClick={() => {
                    selectAllDuplicates(true);
                    selectAllSimilar(true);
                  }}
                  className="text-sm bg-blue-500 hover:bg-blue-600 text-white px-2 py-1 rounded transition-colors flex items-center"
                >
                ...
                <button
                  onClick={() => {
                    selectAllDuplicates(false);
                    selectAllSimilar(false);
                  }}
                  className="text-sm bg-gray-400 hover:bg-gray-500 text-white px-2 py-1 rounded transition-colors flex items-center"
                >
                  <svg className="h-4 w-4 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="space-y-6 overflow-y-auto">
            ...
            {analysisResults.analysis.duplicates && analysisResults.analysis.duplicates.length > 0 && (
              <div className="space-y-3 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                {/* Collapsible header */}
                <div 
                  onClick={() => setDuplicatesExpanded(!duplicatesExpanded)}
                  className="flex justify-between items-center bg-gray-50 dark:bg-gray-800 p-3 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-750 transition-colors"
                >
                  ...
                  <svg 
                    className={`h-5 w-5 text-gray-500 transform transition-transform ${duplicatesExpanded ? 'rotate-180' : ''}`} 
                ...
                {duplicatesExpanded && (
                  <div className="p-4">
                    ...
                    {countMergeEligibleDuplicates() > 0 && (
                      <div className="flex items-center justify-between mb-4 bg-gray-50 dark:bg-gray-800 rounded p-2">
                    ...
                    <div className="space-y-4 max-h-[400px] overflow-y-auto">
                      {analysisResults.analysis.duplicates.map((group: any, index: number) => (
                        // Existing duplicate task group UI remains mostly the same
                        // But with checkbox fix for task merges
                        <div 
                          key={`duplicate-${index}`} 
                          ...
                          {group.recommendedAction === 'merge' && group.mergedTask && (
                            <div>
                                  ...
                                  <div className="flex items-center space-x-2">
                                    <div className="flex items-center bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 text-xs font-medium px-3 py-1 rounded-full">
                                      <input
                                        ...
                                        className="h-4 w-4 text-green-600 focus:ring-green-500 border-green-300 mr-2"
                                      />
                              ...
                              <div className="flex border-x border-b border-gray-200 dark:border-gray-700 rounded-b-lg overflow-hidden">
                                {/* Left side: Original tickets - with fixed height */}
                                <div className="w-1/2 border-r border-gray-200 dark:border-gray-700">
                                  <div className="bg-gray-50 dark:bg-gray-800 p-2 border-b border-gray-200 dark:border-gray-700 sticky top-0">
                                ...
                                {/* Right side: Merged task - with fixed height */}
                                <div className="w-1/2">
                                  <div className="bg-green-50 dark:bg-green-900/20 p-2 border-b border-gray-200 dark:border-gray-700 sticky top-0">
                                    <div className="text-xs font-medium text-green-700 dark:text-green-300">
                                      Merged Result
                                    </div>
                                  </div>
                                  <div className="p-3 max-h-56 overflow-y-auto">
                                    {/* Existing merged task details */}
            ...
            {/* Similar tasks section - follows same pattern as duplicate tasks */}
            {analysisResults.analysis.similar && analysisResults.analysis.similar.length > 0 && (
              <div className="space-y-3 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                ...
                >
                  ...
                  <svg 
                    className={`h-5 w-5 text-gray-500 transform transition-transform ${similarExpanded ? 'rotate-180' : ''}`} 
                    xmlns="http://www.w3.org/2000/svg" 
                    viewBox="0 0 20 20" 
                    fill="currentColor"
                  >
                ...
                {similarExpanded && (
                  <div className="p-4">
                    {/* Rest of similar tasks section follows same pattern as duplicates */}
                    {/* ... similar code as duplicates section ... */}
                  </div>
                )}
              </div>
            )}
          ...
          <div className="sticky bottom-0 bg-white dark:bg-gray-900 pt-2 pb-4 border-t border-gray-200 dark:border-gray-700 mt-4 z-10">
